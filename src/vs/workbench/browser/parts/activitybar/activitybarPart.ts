...
registerThemingParticipant((theme, collector) => {
	...
	// Top/Bottom activity bar hover foreground
	const activityBarTopHoverForegroundColor = theme.getColor(ACTIVITY_BAR_TOP_HOVER_FOREGROUND);
	if (activityBarTopHoverForegroundColor) {
		collector.addRule(`
			...
			.monaco-workbench .part.sidebar > .title > .composite-bar-container > .composite-bar > .monaco-action-bar .action-item:hover .action-label {
				color: ${activityBarTopHoverForegroundColor} !important;
			}
			...
			.monaco-workbench .part.sidebar > .title > .composite-bar-container > .composite-bar > .monaco-action-bar .action-item:hover .action-label.uri-icon {
				background-color: ${activityBarTopHoverForegroundColor} !important;
			}
		`);
	}

	// Top/Bottom activity bar hover background
	const activityBarTopHoverBackgroundColor = theme.getColor(ACTIVITY_BAR_TOP_HOVER_BACKGROUND);
	if (activityBarTopHoverBackgroundColor) {
		collector.addRule(`
			.monaco-workbench .part.sidebar > .header-or-footer > .composite-bar-container > .composite-bar > .monaco-action-bar .action-item:hover::before,
			.monaco-workbench .part.sidebar > .title > .composite-bar-container > .composite-bar > .monaco-action-bar .action-item:hover::before {
				content: "";
				position: absolute;
				top: 0;
				left: 0;
				right: 0;
				bottom: 0;
				background-color: ${activityBarTopHoverBackgroundColor};
				pointer-events: none;
				border-radius: 4px;
			}
		`);
