...
export const enum AccessibilityWorkbenchSettingId {
	...
	FocusFollowsMouseEnabled = 'accessibility.focusFollowsMouse.enabled',
	FocusFollowsMouseDelay = 'accessibility.focusFollowsMouse.delay',
...
export function registerAccessibilityConfiguration() {
	...
	registry.registerConfiguration({
		...
		properties: {
			...
			[AccessibilityWorkbenchSettingId.FocusFollowsMouseEnabled]: {
				description: localize('focusFollowsMouseEnabled', 'Controls whether keyboard focus follows the mouse when hovering over editor groups, the Side Bar, the Panel, and the Secondary Side Bar. Focusing requires the part to be visible and the cursor to remain over it for a brief moment.'),
				type: 'boolean',
				default: false,
				tags: ['accessibility'],
				scope: ConfigurationScope.APPLICATION,
			},
			[AccessibilityWorkbenchSettingId.FocusFollowsMouseDelay]: {
				markdownDescription: localize('focusFollowsMouseDelay', 'The delay in milliseconds before keyboard focus follows the mouse. This will only take effect when {0} is enabled.', `\`#${AccessibilityWorkbenchSettingId.FocusFollowsMouseEnabled}#\``),
				type: 'number',
				minimum: 0,
				default: 200,
				tags: ['accessibility'],
