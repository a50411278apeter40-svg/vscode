...
export class AutomationIsolationGroupActionViewItem extends BaseActionViewItem {
	...
	override render(container: HTMLElement): void {
		...
		this.renderDisposables.add(this.sessionsProvidersService.onDidChangeProviders(() => this.refreshTargetAndRender()));
	...
	private refreshTargetCapability(): void {
		const providerId = this.state.providerId ?? (
			this.isolationModel.folderUri && this.state.sessionTypeId
				? this.sessionsManagementService.getSessionTypesForFolder(this.isolationModel.folderUri)
					.find(candidate => candidate.sessionType.id === this.state.sessionTypeId)?.providerId
				: undefined
		);
		const provider = this.sessionsProvidersService.getProviders().find(candidate => candidate.id === providerId);
		if (!this.state.sessionTypeId || !providerId || !provider) {
		...
		const capabilities = this.state.sessionTypeId ? provider?.getSessionTypeCapabilities?.(this.state.sessionTypeId) : undefined;
		const supportsWorktreeConfiguration = capabilities?.supportsWorktreeConfiguration === true;
