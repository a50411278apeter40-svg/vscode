...
suite('Automation branch picker', () => {
	...
	function createItem(options?: {
		...
		instantiationService.stub(ISessionsProvidersService, upcastPartial<ISessionsProvidersService>({
			...
			getProviders: () => providerAvailable ? [upcastPartial<ISessionsProvider>({
				id: state.providerId ?? 'default-copilot',
				getSessionTypeCapabilities: sessionTypeId => ({
					supportsWorktreeConfiguration: sessionTypeId === 'copilotcli',
				}),
			})] : [],
		}));
		instantiationService.stub(ISessionsManagementService, upcastPartial<ISessionsManagementService>({
			...
			getSessionTypesForFolder: () => [{
				...
				sessionType: { id: 'copilotcli', label: 'Copilot', icon: Codicon.copilot },
			}],
		}));
		...
		return {
			...
			setProviderAvailable: () => {
				...
				providersChanged.fire({ added: [], removed: [] });
			},
		};
	}
