...
the branch control read-only. Agent-host Copilot CLI maps the generic
`workspace` isolation value to its `folder` config value, applies isolation and
branch atomically, and verifies both values in the resolved config. Automation
values are one-shot and do not replace the user's remembered interactive
session defaults. The automation run cancellation token covers this configuration
phase; cancellation disposes the draft before any prompt is dispatched.
