...
suite('WorktreeIsolation', () => {
	...
	test('resolveIsolationConfig advertises folder/worktree + branch based on git state', async () => {
		...
		assert.deepStrictEqual({
			...
			repoWorktreeSelected: { branchDefault: repoWorktreeSelected.branchDefault, branchEnum: repoWorktreeSelected.branchProperty?.protocol.enum },
			...
			repoWorktreeSelected: { branchDefault: 'feature', branchEnum: ['feature'] },
